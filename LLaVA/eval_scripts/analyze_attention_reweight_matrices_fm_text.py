## This script reweights the textual modality
import os
import json
import argparse
import shortuuid
...
def eval_model(args):
    ...
    # Output file naming
    attn_file_parts = [
        f"y25_fmtext_{args.dataset}_attn",
        f"_text_{args.text_alpha}" if args.reweight_text else "",
        f"_to_{args.redistribute_to}" if args.redistribute_to in ['img', 'system', 'both', 'prop'] else "",
        f'_k_{args.top_k}',
    ...
    with open(f"{attn_file_name}.csv", mode="w", newline="") as output_file:
        ...
        for (input_ids, image_tensor, image_sizes), line in tqdm(zip(data_loader, questions), total=len(questions)):
            ...
            model.config.text_alpha = args.text_alpha ##
...
if __name__ == "__main__":
    ...
    parser.add_argument("--text_alpha", type=float, default=1.0) ##
