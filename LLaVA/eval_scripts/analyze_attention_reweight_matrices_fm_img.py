## This script reweights the image modality
import os
import json
import argparse
import shortuuid
...
def eval_model(args):
    ...
    print(f"[DEBUG] args.use_reduced = {args.use_reduced}")
    ...
    # Output file naming
    attn_file_parts = [
        f"y25_fmimg_{args.dataset}_attn",
        f"_img_{args.img_alpha}" if args.reweight_img else "",
        f"_to_{args.redistribute_to}" if args.redistribute_to in ['system', 'text', 'both', 'prop'] else "",
        f'_k_{args.top_k}',
    ...
    with open(f"{attn_file_name}.csv", mode="w", newline="") as output_file:
        ...
        for (input_ids, image_tensor, image_sizes), line in tqdm(zip(data_loader, questions), total=len(questions)):
            ...
            model.config.img_alpha = args.img_alpha ##
            ...
            for stage in ["pre_alpha", "post_alpha"]:
                ...
                means_system = np.mean([obj_to_float(s["system_mean"]) for s in all_layer_stats])
                means_img = np.mean([obj_to_float(s["img_mean"]) for s in all_layer_stats])
                means_text = np.mean([obj_to_float(s["text_mean"]) for s in all_layer_stats])

                props_img = np.mean([obj_to_float(s["img_prop"]) for s in all_layer_stats])
                props_sys = np.mean([obj_to_float(s["system_prop"]) for s in all_layer_stats])
                props_txt = np.mean([obj_to_float(s["text_prop"]) for s in all_layer_stats])

                var_img = np.var([obj_to_float(s["img_mean"]) for s in all_layer_stats])
                var_sys = np.var([obj_to_float(s["system_mean"]) for s in all_layer_stats])
                var_txt = np.var([obj_to_float(s["text_mean"]) for s in all_layer_stats])
            ...
            if attentions is not None and len(attentions) > 0:
                ...
                for layer_idx, attn in enumerate(attn_list):
                    ...
                    for to_modality in MODALITY_LABELS:
                        ...
                        # Compute mean attention weights for each head (average over all queries and target tokens)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--redistribute_to", type=str, default='system') ##
    parser.add_argument("--img_alpha", type=float, default=1.0) ##
    ...
    if args.reweight_img:
        reweight_parts.append(f'img{args.img_alpha}')
    if args.redistribute_to in ['system', 'text', 'both', 'prop']:
        reweight_parts.append(f'to_{args.redistribute_to}')

    reweight_str = '_'.join(reweight_parts) if reweight_parts else 'none'
    dataset_type = "reduced" if args.use_reduced else "full"
    answers = f'./results/{args.dataset}/llava_3000/{args.dataset}_fmimg_{reweight_str}_k_{args.top_k}_heads_{args.attention_head_path.split("/")[-1].replace(".json","")}_{dataset_type}_seed{args.seed}.jsonl'
